...
DATA_DIR = BASE_DIR / "data"   # cnn_image/data
...
def preprocess_image(image, label):
    """Convierte a float32 y normaliza a [0, 1]."""
    image = tf.cast(image, tf.float32) / 255.0
    return image, label


def prepare_dataloader(ds: tf.data.Dataset, training: bool = False) -> tf.data.Dataset:
    """Aplica shuffle (si training), preprocesamiento y prefetch."""
    if training:
...
def load_rps_datasets():
    """
    Carga el dataset Rock–Paper–Scissors desde carpetas locales
    y lo separa en entrenamiento (80%) y validación (20%).
    """
...
def build_cnn_model(input_shape=(160, 160, 3)) -> keras.Model:
    """
    CNN con al menos tres capas de convolución.
    Salida con 3 clases (paper, rock, scissors).
    """
...
def train_model(model, train_ds, val_ds, epochs=5):
    """Entrena el modelo y devuelve el history."""
    callbacks = [
...
def save_model(model, name: str = "cnn_rps.keras") -> Path:
    """Guarda el modelo entrenado en cnn_image/models/ y devuelve la ruta."""
    # Si el nombre no tiene extensión válida, le agregamos .keras
...
def main():
    ...
    # 2) Preparar dataloaders con preprocesamiento
